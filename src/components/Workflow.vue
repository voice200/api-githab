...
<div class="workflow_container">
  ...
  <div class="date-interval">
    <div class="date-start">
      <select name="dayStart" id="dayStart" :disabled="!getBranches.length" v-model="dateStartUser.day">
        <option
          v-for="(day, i) in dayItems" :key="i"
          :value="day">{{ day }}</option>
      </select>
      <select name="monthStart" id="monthStart" :disabled="!getBranches.length" v-model="dateStartUser.month" >
        <option
          :disabled="month < new Date(getSearch.dateStart).getMonth()"
          v-for="(month, i) in monthItems" :key="i"
          :value="month">{{ month }}</option>
      </select>
      <select name="yearStart" id="yearStart" :disabled="!getBranches.length" v-model="dateStartUser.year">
        <option
          v-for="(year, i) in yearItems" :key="i"
          :value="year">{{ year }}</option>
      </select>
    </div>
    <div class="date-end">
      <select name="dayEnd" id="dayEnd" :disabled="!getBranches.length" v-model="dateEndUser.day">
        <option
          v-for="(day, i) in dayItems" :key="i"
          :value="day">{{ day }}</option>
      </select>
      <select name="monthEnd" id="monthEnd" :disabled="!getBranches.length" v-model="dateEndUser.month">
        <option
          v-for="(month, i) in monthItems" :key="i"
          :value="month">{{ month }}</option>
      </select>
      <select name="yearEnd" id="yearEnd" :disabled="!getBranches.length" v-model="dateEndUser.year">
        <option
          v-for="(year, i) in yearItems" :key="i"
          :value="year">{{ year }}</option>
      </select>
    </div>
    <button class="workflow_button" :disabled="!getBranches.length" @click="changeDate">Save Date</button>
  </div>
  <hr/>
  <button class="workflow_button" :disabled="disableAllData" @click="getAllDataSearch">Choose</button>
...
name: "workflow",
  ...
  methods: {
    ...
    changeDate() {
      const startDate = Object.entries(this.dateStartUser).reverse().map(item => item[1])
      const endDate = Object.entries(this.dateEndUser).reverse().map(item => item[1])
